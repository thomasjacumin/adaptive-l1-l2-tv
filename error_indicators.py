...
def L1L2TV_dualgap_error_scalar(mesh, v, q1, q2, g, lambdaa, alpha1, alpha2, beta, B, T, TAdj, S, gamma1, gamma2, dofs, elements):
    ...
    ## L2-Regul S
    term5 = mK*beta/2*Sv**2
    ## F2*
    if lambdaa.all() > 0:
        term11 = mK*lambdaa*hubert_vec(rho2N(mesh, gradV), gamma2)
    ## F1*
    if alpha1.all() > 0:
        term10 = mK*alpha1*hubert_vec(np.abs(Tv-g), gamma1)

    # Dual
    ## B^-1-Data term
    term7 = -mK/2*L2DualTerms[0:dofs]*vFromDualVariable[0:dofs]
    ## g**2
